...
class Cipa implements Serializable {
		...
		script.node(node.nodeLabel) {
			...
			for (tool in tools) {
				...
				if (tool.is(toolMvn)) {
					...
					envVars.add("MAVEN_OPTS=-Dmaven.multiModuleProjectDirectory=\"${toolHome}\" ${toolMvn.options} ${env.MAVEN_OPTS}")
				}
	...
	String mvn(
		...
		script.withEnv(["MAVEN_OPTS=${optionsString} ${env.MAVEN_OPTS}"]) {
